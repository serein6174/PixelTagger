...
仓库当前没有 `AGENTS.md` 和项目级 `.codex/` 配置，`.agents/` 目录也未包含规则或技能文件。项目实际采用的约束来自下列架构与模块交接文档：

| 文档                                                                                           | 本项目采用的约束                                                                                                                                                                                                                                               |
| ---------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`docs/mvvm.md`](../mvvm.md)                                                                   | 规定 View 只处理控件、输入、绘制和坐标转换，不直接修改 `ProjectModel`；ViewModel 执行业务并通过受控接口修改 Model；Model 不依赖 `QWidget`，标注只保存原图坐标。大对象变化通过语义明确的无参 signal 通知，由 `Application` 调用 ViewModel getter 拉取展示数据。 |
| [`docs/annotation-category-edit-team-handoff.md`](../annotation-category-edit-team-handoff.md) | 规定 Canvas 在 View 层完成命中测试和坐标转换，不能把 Widget 坐标交给 Model/ViewModel；`Application` 只连接请求和转发展示数据，不直接修改标注类别。文档同时给出联调顺序、自动测试和共享文件的冲突提醒。                                                         |
| [`docs/image-processing-team-handoff..md`](../image-processing-team-handoff..md)               | 规定处理预览不覆盖项目原图，OpenCV 算法和参数校验留在 Processor/ViewModel；View 只发出操作意图，`Application` 不实现算法或修改项目图片、标注。                                                                                                                 |
| [`docs/yolo-export-team-handoff.md`](../yolo-export-team-handoff.md)                           | 规定 View 只提交导出目录，YOLO 格式与文件系统规则由 Exporter/`ProjectViewModel` 处理；`Application` 仅负责对象装配和信号连接，不能在导出前自行修改 Model。                                                                                                     |

`docs/experiment-report-mid.md` 记录了 Command 方案被 `Application + signals/slots` 取代、导入职责回收到 `ImageViewModel` 等架构演化，可作为决策背景，但不把阶段报告本身当作强制规则。代码或智能体建议与上述边界不一致时，需要先由成员确认是否修改规则，不能为快速实现功能而绕开分层。
